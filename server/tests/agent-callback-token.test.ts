...
describe("the run assertion", () => {
  ...
  test("keeps a durable task and channel inside the signed assertion", () => {
    const scoped = {
      ...RUN,
      taskRunId: "task-1",
      channelId: "channel-1",
    };
    expect(readRunAssertion(mintRunAssertion(scoped, KEY), KEY)).toEqual(
      scoped,
    );
  });

  test("refuses a partial durable task scope", () => {
    const signed = mintRunAssertion({ ...RUN, taskRunId: "task-1" }, KEY);
