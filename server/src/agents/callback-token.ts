...
export type RunAssertion = {
  ...
  runId: string;
  /** The durable Kayco task, when this turn came from a channel. */
  taskRunId?: string;
  /** The durable channel that owns `taskRunId`; the approval service verifies the pair. */
  channelId?: string;
};
...
export function readRunAssertion(
  ...
  try {
    ...
    if (
      ...
      (payload.taskRunId !== undefined &&
        typeof payload.taskRunId !== "string") ||
      (payload.channelId !== undefined &&
        typeof payload.channelId !== "string") ||
      Boolean(payload.taskRunId) !== Boolean(payload.channelId) ||
    ...
    return {
      ...
      runId: payload.runId,
      ...(payload.taskRunId ? { taskRunId: payload.taskRunId } : {}),
...
export async function authoriseAgentCall(options: {
  ...
  return {
    ...
    ...(assertion.taskRunId ? { taskRunId: assertion.taskRunId } : {}),
    ...(assertion.channelId ? { channelId: assertion.channelId } : {}),
  };
}
