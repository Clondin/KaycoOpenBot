 ...
 *
 * The graph owns the model's server-side tool loop. Governed MCP calls return to the OpenBot server,
 * while tools that genuinely need the visible surface, such as components and computer controls,
 * remain frontend calls. That lets unattended work use MCP without pretending a browser is open.
 */
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        // completed model event, which is precisely the plumbing agent-bot does by hand.
